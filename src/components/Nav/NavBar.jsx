  ...
  return (
    <div
      className={`nav-container w-full sticky top-0 z-[99] h-[52px] shadow-lg flex items-center justify-center px-[1rem] md:px-[6rem] text-white transition-all duration-300 ease-in-out ${
        scrolled
          ? "bg-black/30 bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-50 shadow-lg"
          : "bg-[#000000]"
      }`}
    >
      <div className="navbar w-full flex items-center justify-between">
        ...
        >
          <div className="w-full">
            <ul className="min-w-[18rem] gap-5 md:gap-0 flex flex-col md:flex-row justify-between items-center px-2">
              {tabs.map((item) => {
                return (
                  <Link
                    onClick={() => setMenuToggle((prev) => !prev)}
